...
const Study: React.FC = () => {
  ...
  // useEffect hook para manejar la carga inicial de datos y la actualización periódica.
  useEffect(() => {
    // Función asíncrona para obtener mazos y tarjetas.
    const fetchDecksAndCards = async () => {
      try {
        // Realiza solicitudes HTTP para obtener todos los mazos y tarjetas.
        ...
        // Almacena los datos obtenidos en variables.
        const allDecks: Deck[] = decksResponse.data;
        const allCards: Card[] = cardsResponse.data;

        // Función para procesar los mazos, calculando métricas de estudio.
        const processDecks = (currentCards: Card[], currentDecks: Deck[]) => {
          const currentTime = Date.now(); // Obtiene el tiempo actual.
          return currentDecks.map(deck => { // Itera sobre cada mazo.
            // Filtra las tarjetas que pertenecen al mazo actual.
            const cardsInDeck = currentCards.filter(card => card.deckId === deck._id);
            // Calcula las métricas de estudio (tarjetas para estudiar, revisadas, y tiempo de próxima revisión).
            ...
            // Formatea el tiempo restante para la próxima revisión, si existe.
            const nextReviewTimeRemaining =
              minNextReviewTime !== Infinity
                ? formatTimeRemaining(minNextReviewTime - currentTime)
                : undefined;
            
            // Retorna un nuevo objeto mazo con las métricas de estudio añadidas.
            return { ...deck, cardsForStudy, cardsReviewed, nextReviewTimeRemaining };
          });
        };

        // Procesa los mazos iniciales para enriquecerlos con las métricas de estudio.
        const enrichedDecks = processDecks(allCards, allDecks);

        // Ordena los mazos por la cantidad de tarjetas para estudiar (de menor a mayor).
        enrichedDecks.sort((a, b) => (a.cardsForStudy || 0) - (b.cardsForStudy || 0));

        // Actualiza el estado de los mazos.
        setDecks(enrichedDecks);

        // Establece un intervalo para actualizar periódicamente las métricas de los mazos.
    ...
    // Llama a la función para obtener y procesar los mazos y tarjetas al montar el componente.
    fetchDecksAndCards();
  ...
  const DeckTile: React.FC<{ deck: Deck }> = ({ deck }) => {
    // `textRef` se utiliza para referenciar el elemento `p` que muestra el nombre del mazo, permitiendo ajustar dinámicamente el tamaño de la fuente.
    const textRef = useRef<HTMLParagraphElement>(null);

    return (
      // Componente Link que envuelve la tarjeta del mazo, permitiendo la navegación al hacer clic.
      // Aplica estilos Tailwind CSS para el diseño y estilos en línea para el color del borde y fondo.
      <Link
      ...
      >
        {/* Contenedor para el icono del mazo. */}
        <div className="flex items-center justify-center rounded-lg bg-[#f0f2f4] shrink-0 size-10">
          {/* Icono SVG que representa un mazo de cartas. El color de relleno se adapta al color del mazo. */}
          <svg
            width="24px"
            height="24px"
        ...
        {/* Párrafo para mostrar el nombre del mazo. Utiliza una referencia para el ajuste de fuente dinámico y aplica estilos de texto. */}
        <p ref={textRef} className="text-white font-bold leading-normal truncate w-full text-xl">{deck.name}</p>
        {/* Contenedor para la información de estudio del mazo. */}
