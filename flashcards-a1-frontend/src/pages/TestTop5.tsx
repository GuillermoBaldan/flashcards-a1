  ...
  useEffect(() => {
    const fetchCards = async () => {
      try {
        ...
        const top5Percent = Math.ceil(cardsForStudy.length * 0.05);
        const easyCards = cardsForStudy.slice(0, top5Percent);
        setCards(easyCards);
      } catch (err) {
  ...
  if (cards.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
        <h1 className="text-3xl font-bold mb-8">Test del Top 5% de Preguntas Más Fáciles</h1>
